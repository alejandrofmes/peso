<div class="w-full h-full p-6 bg-white rounded-lg shadow">
    <div class="mb-10">
        <div class="flex flex-col justify-between w-full gap-2 md:flex-row">
            ...
            <h1 class="text-2xl font-bold">Job Posting Trends</h1>

            <button type="button" x-data=""
                x-on:click.prevent="$dispatch('open-modal', 'filter-jobposting-trends-modal')"
    ...
    <div class="flex items-end h-full">
        <livewire:livewire-line-chart key="{{ $jobPostingTrend->reactiveKey() }}" :line-chart-model="$jobPostingTrend" />
    </div>


    <x-modal name="filter-jobposting-trends-modal" focusable>
        <div class="items-center w-full max-w-4xl px-6 py-6">
            <h2 class="text-lg font-medium text-gray-900">
                {{ __('Filter Job Posting Trends') }}
            </h2>
            <hr>
                                    ...
                                    @foreach (range(1, 12) as $month)
                                        <div class="flex items-center p-2">
                                            <input wire:model='mountSelectedMonths' type="checkbox"
                                                id="jobpostingTrends-{{ $month }}" value="{{ $month }}"
                                                class="w-4 h-4 text-blue-600 border-gray-300 rounded" />
                                            <label for="jobpostingTrends-{{ $month }}"
            ...
            <div class="flex justify-between mt-6">
                <x-secondary-button x-on:click="$dispatch('close-modal', 'filter-jobposting-trends-modal')">
                    {{ __('Cancel') }}
                </x-secondary-button>
